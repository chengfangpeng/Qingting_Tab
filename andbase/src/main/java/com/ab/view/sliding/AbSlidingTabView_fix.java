 ...
 * 
 * 修改AbFragmentPagerAdapter为AbFragmentPagerStateAdapter
 */
@SuppressLint("NewApi")
public class AbSlidingTabView_fix extends LinearLayout {
	...
	/** tab选中的背景. */
	private int tabBackgroundResourceSelect = -1;
	...
	/** tab的选中文字的大小 */
	private int tabSelectTextSize = 30;
	...
	/** 内容区域的适配器. */
	private AbFragmentPagerStateAdapter mFragmentPagerAdapter = null;
	 ...
	 *            the context
	 */
	public AbSlidingTabView_fix(Context context) {
		this(context, null);
	}
	 ...
	 *            the attrs
	 */
	public AbSlidingTabView_fix(Context context, AttributeSet attrs) {
		super(context, attrs);
	...
	public AbSlidingTabView_fix(Fragment fragment) {
		...
		if (VERSION.SDK_INT <= 17) {
			AbLogUtil.e(AbSlidingTabView_fix.class, "AbSlidingTabView(Fragment fragment) 要求最低SDK版本17");
			return;
		}
	...
	public void initView() {
		...
		if (!(this.context instanceof FragmentActivity)) {
			AbLogUtil.e(AbSlidingTabView_fix.class, "构造AbSlidingTabView的参数context,必须是FragmentActivity的实例。");
		}
	}
	...
	public void setCurrentItem(int item) {
		...
		for (int i = 0; i < tabCount; i++) {
			...
			if (isSelected) {
				if (tabBackgroundResourceSelect != -1)
					child.setTabBackgroundResource(tabBackgroundResourceSelect);
				child.setTabTextColor(tabSelectColor);
				child.setTabTextSize(tabSelectTextSize);
				animateToTab(item);
			} else {
				if (tabBackgroundResource != -1)
					child.setTabBackgroundResource(tabBackgroundResource);
				child.setTabTextColor(tabTextColor);
				child.setTabTextSize(tabTextSize);
			}
		}
	}
	...
	public void setTabSelectTextSize(int tabTextSize) {
		this.tabSelectTextSize = tabTextSize;
	}
	 ...
	 *            the new tab background resource
	 */
	public void setTabBackgroundResourceSelect(int resid) {
		tabBackgroundResourceSelect = resid;
	}
